...
class RoleService(BaseService[Role]):
    ...
    def get_roles(self, filters: Dict[str, Any], page: int = 1, page_size: int = 100) -> Tuple[List[Dict[str, Any]], int]:
        """
        获取角色列表
        ...
        Returns:
            Tuple[List[Dict[str, Any]], int]: 角色列表和总数
        """
        try:
            ...
            if filters.get("role_type"):
                query = query.filter(Role.role_type == filters["role_type"])
            ...
            # 获取分页数据
            roles = query.order_by(desc(Role.id)).offset(offset).limit(page_size).all()
    ...
    def get_role_user_count(self, role_id: int) -> int:
        ...
        try:
            ...
            logger.error(f"获取角色 {role_id} 关联用户数量失败: {str(e)}")
            return 0