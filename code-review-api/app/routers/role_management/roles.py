...
async def get_roles(
    ...
    Args:
        ...
        name (str, optional): 角色名称筛选
        role_type (str, optional): 角色类型筛选
        page (int): 页码
        page_size (int): 每页记录数
        db (Session): 数据库会话
    ...
    logger.info(f"用户 {current_user.username} 请求获取角色列表，页码: {page}，每页记录数: {page_size}，名称筛选: {name}，角色类型: {role_type}，IP: {client_ip}")
    ...
    try:
        ...
        # 获取角色列表
        roles, total = service.get_roles(filters, page, page_size)
        
        # 格式化角色数据
        formatted_roles = []
        for role_dict in roles:
            # 获取角色的项目数量和用户数量
            project_count = service.get_role_project_count(role_dict["id"])
            user_count = service.get_role_user_count(role_dict["id"])
            
            # 添加额外的字段
            role_dict["project_count"] = project_count
            role_dict["user_count"] = user_count
            
            formatted_roles.append(role_dict)
        
        process_time = time.time() - start_time
        logger.info(f"获取角色列表成功，共 {total} 条记录，处理时间: {process_time:.2f}秒")
